...
async def get_vulnerabilities_by_ecosystem(db: AsyncIOMotorDatabase, ecosystem: str) -> List[OSVVulnerability]:
    return await mongo.find_many(db, collection, {"affected": {"$elemMatch": {"package.ecosystem": ecosystem}}})
...
async def get_vulnerabilities_by_package(db: AsyncIOMotorDatabase, package: str) -> List[OSVVulnerability]:
    return await mongo.find_many(db, collection, {"affected": {"$elemMatch": {"package": {"$elemMatch": {"name": package}}}}})